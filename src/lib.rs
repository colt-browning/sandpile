...
#[derive(Debug)]
pub struct FiniteGridSandpile<'a>(&'a GridSandpile);

impl<'a> FiniteGridSandpile<'a> {
	pub fn as_ref(&self) -> &GridSandpile {
		self.0
	}
}
...
impl<'a> FiniteGridSandpile<'a> {
	pub fn neutral(grid_type: GridType, neighbourhood: Neighbourhood, (x, y): (usize, usize)) -> Box<GridSandpile> {
		...
		if grid_type == GridType::Rectangular && neighbourhood == Neighbourhood::VonNeumann && x % 2 == 0 && y == x && x >= 6 {
			return FiniteGridSandpile::neutral_finite_vn_es_optimized(x/2)
		}
	...
	fn neutral_finite_vn_es_optimized(x: usize) -> Box<GridSandpile> { // es = even square
		...
		GridSandpile::from_grid(GridType::Rectangular, Neighbourhood::VonNeumann, grid).unwrap()
	}
	...
	pub fn inverse(&self) -> Box<GridSandpile> {
		let s = self.as_ref();
		if let GridType::Infinite(..) = s.grid_type {
			panic!()
		}
		let t = 2 * (s.neighbourhood.neighbours() - 1);
		let mut sandpile = GridSandpile::from_grid(s.grid_type, s.neighbourhood, vec![vec![t; s.grid[0].len()]; s.grid.len()]).unwrap();
		for y in 0..s.grid.len() {
			for x in 0..s.grid[0].len() {
				sandpile.grid[y][x] = 2 * (t - sandpile.grid[y][x]) - s.grid[y][x];
			}
		}
		sandpile.topple();
		sandpile
...
mod tests {
	...
	#[test]
	fn id_finite() {
		let s = FiniteGridSandpile::neutral(GridType::Rectangular, Neighbourhood::VonNeumann, (3, 2));
		let g = s.into_grid();
	...
	fn id_finite_optimized() {
		let s = FiniteGridSandpile::neutral(GridType::Rectangular, Neighbourhood::VonNeumann, (6, 6));
		let g = s.into_grid();
		assert_eq!(g, vec![
			vec![2, 1, 3, 3, 1, 2],
			vec![1, 2, 2, 2, 2, 1],
		...
		let s = FiniteGridSandpile::neutral(GridType::Rectangular, Neighbourhood::VonNeumann, (10, 10));
		let g = s.into_grid();
		assert_eq!(g, vec![
	...
	#[test]
	fn id_torus() {
		let s = FiniteGridSandpile::neutral(GridType::Toroidal, Neighbourhood::VonNeumann, (3, 2));
		let g = s.into_grid();
	...
	fn from_string() {
		let st = "&. \n:.:\n";
		let s = GridSandpile::from_string(GridType::Rectangular, Neighbourhood::VonNeumann, (3, 2), String::from(st)).unwrap();
		let g = s.into_grid();
	...
	fn display() {
		...
		let s = GridSandpile::from_grid(GridType::Rectangular, Neighbourhood::VonNeumann, g.clone()).unwrap();
	...
	#[test]
	fn add() {
		let mut s1 = GridSandpile::from_grid(GridType::Rectangular, Neighbourhood::VonNeumann, vec![vec![2, 1, 2], vec![3, 3, 1], vec![2, 3, 1]]).unwrap();
		let r = s1.clone();
		let s2 = GridSandpile::from_grid(GridType::Rectangular, Neighbourhood::VonNeumann, vec![vec![2, 1, 2], vec![1, 0, 1], vec![2, 1, 2]]).unwrap();
		s1.add(&s2).unwrap();
		assert_eq!(s1, r);
	...
	#[test]
	fn order() {
		let s = GridSandpile::from_grid(GridType::Rectangular, Neighbourhood::VonNeumann, vec![vec![3, 3, 3], vec![3, 3, 3]]).unwrap();
		assert_eq!(s.order(), 7);
	}
	...
	fn inverse() {
		let s = GridSandpile::from_grid(GridType::Rectangular, Neighbourhood::VonNeumann, vec![vec![3, 3, 3], vec![3, 3, 3]]).unwrap();
		let i = GridSandpile::from_grid(GridType::Rectangular, Neighbourhood::VonNeumann, vec![vec![2, 3, 2], vec![2, 3, 2]]).unwrap();
	...
	fn moore() {
		assert_eq!(
			GridSandpile::from_grid(GridType::Rectangular, Neighbourhood::Moore, vec![vec![0, 0, 0], vec![0, 9, 0], vec![0, 0, 0]]).unwrap(),
			GridSandpile::from_grid(GridType::Rectangular, Neighbourhood::Moore, vec![vec![1, 1, 1], vec![1, 1, 1], vec![1, 1, 1]]).unwrap()
		);
	}
}
